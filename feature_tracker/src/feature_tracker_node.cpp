...
void callback(const sensor_msgs::ImageConstPtr &img_msg, const sensor_msgs::ImageConstPtr &mask_msg)
{
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        if (SHOW_TRACK)
        {
            ...
            {
                // if(i == 0){
                //     mask_img = trackerData[0].mask.clone();
                //     cv::cvtColor(mask_img, mask_img, CV_GRAY2RGB);
                // }
...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::Image> mask_sub(n, MASK_TOPIC, 100);

    message_filters::TimeSynchronizer<sensor_msgs::Image, sensor_msgs::Image> sync(image_sub, mask_sub, 10);
